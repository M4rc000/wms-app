...
public function generateNewWipMaterialNo() {
	$result = $this->db->query("SELECT Id, Material_no FROM wip_material ORDER BY Id DESC LIMIT 1")->result_array();
	$lastMaterialNo = isset($result[0]['Material_no']) ? $result[0]['Material_no'] : null;
	if (empty($lastMaterialNo)) {
		return "WP0001";
	}
	$numberPart = substr($lastMaterialNo, 2);
	$newNumber = intval($numberPart) + 1;
	$newMaterialNo = "WP" . str_pad($newNumber, strlen($numberPart), "0", STR_PAD_LEFT);
	return $newMaterialNo;
}
